      ...
      {TABS.map((t) => {
        ...
        return (
          <button key={t.key} onClick={() => onSelect(t.key)} className="flex flex-1 items-center justify-center">
            {isActive ? (
              <span className="flex items-center gap-1.5 rounded-full bg-cream dark:bg-[#2a1b4d] px-4 py-2 text-xs font-bold leading-none text-brand-700 dark:text-brand-300">
...
export default function App() {
  ...
  return (
    <div className="flex h-full flex-col bg-cream dark:bg-[#2a1b4d]">
      <header
              ...
              <button
                ...
                className={`flex flex-1 items-center justify-center gap-1.5 px-4 text-xs font-bold leading-none transition-all duration-200 sm:flex-none sm:text-sm ${
                  ...
                  !showSettings && tab === t.key
                    ? `bg-cream dark:bg-[#2a1b4d] text-brand-700 dark:text-brand-300 ${scrolled ? 'rounded-full' : 'rounded-t-2xl'}`
