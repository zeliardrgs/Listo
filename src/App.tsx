    ...
    <div className="flex h-full flex-col bg-cream">
      ...
      >
        <div className="flex flex-col gap-2 sm:flex-row sm:items-stretch sm:justify-between">
          <div className={`flex items-center justify-between gap-2 sm:justify-start ${scrolled ? 'pb-1' : 'pb-2'}`}>
            <Logo compact={scrolled} />
            <SettingsButton onClick={() => setShowSettings(true)} className="sm:hidden" />
          </div>

          <nav className={`flex gap-1 transition-all duration-200 ${scrolled ? 'items-center' : 'items-stretch'}`}>
            {TABS.map((t) => (
              <button
                key={t.key}
                onClick={() => {
