    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 px-4 sm:items-center" onClick={onClose}>
      ...
      >
        <div className="border-b border-slate-100 px-5 py-4">
          <h2 className="text-base font-extrabold text-slate-800">Ingrédients de « {recipeName} »</h2>
        ...
        <ul className="max-h-[60vh] space-y-1.5 overflow-y-auto px-5 py-3">
          {results.map((r) => {
            ...
            return (
              <li key={r.ingredientName} className="rounded-xl bg-slate-50 px-3 py-2">
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate text-sm font-semibold text-slate-700">{r.ingredientName}</span>
                  {isNew ? (
                  ...
                  <>
                    {pickerFor === r.createdItemId ? (
                      <div className="mt-2 space-y-1">
                        <div className="flex items-center gap-1.5 rounded-lg bg-white px-2.5 py-1.5 ring-1 ring-slate-200">
