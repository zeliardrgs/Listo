  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center gap-1 py-2 pr-3 sm:justify-start sm:px-3 sm:py-2.5">
        <p className="truncate text-sm font-bold text-slate-700 dark:text-slate-200">{recipe.name}</p>
        {recipe.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 sm:mt-1.5">
            {recipe.tags.slice(0, 3).map((tag) => (
              <span key={tag} className="rounded-full bg-brand-50 dark:bg-brand-900/40 px-2 py-0.5 text-[10px] font-semibold text-brand-700 dark:text-brand-300">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="mt-1 flex items-center gap-1.5 sm:mt-2.5">
          ...
          <button
            ...
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-brand-50 dark:bg-brand-900/40 text-brand-600 dark:text-brand-300 hover:bg-brand-100 dark:hover:bg-brand-900/50"
          >
